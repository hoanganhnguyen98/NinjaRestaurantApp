...
import { Container, List, ListItem, Text, Left, Body, Right, Header, Button } from 'native-base';
...
export default class HistoryDetail extends Component {
    ...
    getData = () =>
    {
        fetch(Urls.APIUrl+'cart/history/detail/'+this.props.navigation.getParam('historyId'))
            ...
            <View style={Styles.menu.foodList}>
                <Header style={{backgroundColor: "#ffffff"}}>
                    ...
                    <Right>
                        {this.state.status === 'new' ?
                            <Text>Ordered in {this.props.navigation.getParam('updateTime')}</Text>
                        : this.state.status === 'done' ?
                            <Text>Recevied in {this.props.navigation.getParam('updateTime')}</Text>
                        :
                            <Text>Delivering in {this.props.navigation.getParam('updateTime')}</Text>
                        }
                    </Right>
                </Header>
                <List style={{backgroundColor: "#ffffff"}}>
                    <ListItem icon>
                        <Left>
                            <Button style={{ backgroundColor: "#ffffff" }}>
                                <MCIcon name='account-check' color={Colors.appColor} size={20} />
                            </Button>
                        </Left>
                        <Body>
                    ...
                    <ListItem icon>
                        <Left>
                            <Button style={{ backgroundColor: "#ffffff" }}>
                                <MCIcon name='truck-delivery' color={Colors.appColor} size={20} />
                            </Button>
                        </Left>
                        <Body>
                ...
                <Container>
                    <List>
                        {isLoading ? <ActivityIndicator/> : (
                            <FlatList
                                data={data.sort((after, before) => after.food_name.localeCompare(before.food_name))}
