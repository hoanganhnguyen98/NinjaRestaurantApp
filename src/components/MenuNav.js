...
import { Container, Tab, Tabs, Header, Button, Left, Right, Body, Title, Text, Icon, List, ListItem, Item, Input } from 'native-base';
    ...
    renderItem=(data)=>
        <ListItem thumbnail noBorder onPress={() =>this.props.navigation.navigate('FoodDetail',{
            ...
            <Body>
                ...
                <Text note numberOfLines={1}>{data.item.source}</Text>
                <Text note numberOfLines={1}>{data.item.material}</Text>
                <NumberFormat
            ...
            <Container>
                <Header>
                    ...
                    <Right>
                        <Button transparent onPress={this.toggleSearch}>
                            <MCIcon name={this.state.activeSearch ? 'filter-remove' : 'filter'} size={25} style={{color: '#ffffff'}} />
                        </Button>
                ...
                {!this.state.activeSearch ? null :
                    <Header searchBar rounded>
                        <Item>
                            <FAIcon name="filter" color={Colors.appColor} size={20} style={{marginLeft: 10}} />
                            <Input placeholder="... enter food name" onChangeText={(food)=>this.SearchFilterFunction(food)} />
                        </Item>
                    </Header>
                }
                        ...
                        <List>
                            {isLoading ? <ActivityIndicator/> : (
                                <FlatList
                                    data={
                                        this.state.index == 0 ?
                                        this.state.data.sort((after, before) => after.name.localeCompare(before.name)) :
                                        this.state.index == 1 ?
                                        this.state.data.sort((after, before) => before.name.localeCompare(after.name)) :
                                        this.state.index == 2 ?
                                        this.state.data.sort((after, before) => after.price.localeCompare(before.price)) :
                                        this.state.index == 3 ?
                                        this.state.data.sort((after, before) => before.price.localeCompare(after.price)) :
                                        this.state.data
                                    }
